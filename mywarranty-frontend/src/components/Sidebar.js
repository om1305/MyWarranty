  ...
  return (
    <Sidebar collapsed={collapsed}>
      ...
      <Menu iconShape="square">
        ...
        <MenuItem icon={<FaPlus />} component={<Link to="/add-warranty" />}>
          Add Warranty
        </MenuItem>
        <MenuItem
          icon={<FaCalendarTimes />}
          component={<Link to="/expired-warranties" />}
        >
          Expired Warranties
        </MenuItem>
        <MenuItem icon={<FaCog />} component={<Link to="/settings" />}>
          Settings
        </MenuItem>
      </Menu>
      ...
      <div style={{ padding: '20px', borderTop: '1px solid #eee' }}>
        <div className="d-flex align-items-center mb-2">
          ...
          <div>
            <h5>{user ? `${user.firstName} ${user.lastName}` : 'Guest'}</h5>
            <p className="text-muted mb-0">
              User — {user?.isPremium ? 'Premium Customer' : 'Free Customer'}
            </p>
          </div>
        </div>
